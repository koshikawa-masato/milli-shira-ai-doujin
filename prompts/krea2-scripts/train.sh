...
CMD="accelerate launch --num_cpu_threads_per_process 1 --mixed_precision bf16 \
  ...
  --dataset_config ~/krea2/dataset.toml --sdpa --mixed_precision bf16 \
...
START_JSON=$(python3 - "$RUN" "$STEPS" "$EVERY" "$DIM" "$LR" "$SWAP" "$NOTE" "$DATASET_JSON" "$CMD" <<'PY'
import json,sys
run,steps,every,dim,lr,swap,note,ds,cmd=sys.argv[1:]
print(json.dumps({"event":"train_start","run":run,"dataset_folder":"dataset","dataset":json.loads(ds),
...
T0=$(date +%s)
echo "== cache latents =="
python src/musubi_tuner/krea2_cache_latents.py --dataset_config ~/krea2/dataset.toml --vae ~/krea2/models/qwen_image_vae.safetensors
echo "== cache text encoder =="
python src/musubi_tuner/krea2_cache_text_encoder_outputs.py --dataset_config ~/krea2/dataset.toml --text_encoder ~/krea2/models/qwen3vl_4b_bf16.safetensors --batch_size 1
echo "== train =="
STATUS=done
